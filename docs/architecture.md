...
**Superseded note (Phase 4 Checkpoint D.3, accepted; [ADR 0017](adr/0017-phase4-checkpoint-d3-travel-search-specialist.md)):** System A's bounded ReAct orchestration (Checkpoint D.0, [ADR 0013](adr/0013-phase4-checkpoint-d0-system-a-react-core.md)) is internally two agents, not one flat loop. A **supervisor** ReAct graph owns delegation, the System B A2A call, and final synthesis — it can no longer select `search_flights`/`get_weather`/`web_search`/`search_stays`/`estimate_fair_price` directly. An internal **Travel Search specialist** — a genuinely separate, compiled LangGraph `StateGraph` (`services/planner-a/phase4/specialist.py`), invoked in-process from inside the supervisor's own Execute step, never a second service and never A2A — owns exactly those five actions. Each agent has its own prompt, its own graph state, its own `DecisionProvider` object, and its own closed action allowlist; the two are told apart by which object was constructed for which role, never by inspecting prompt text. The global 60-second deadline, the 8-call external-tool budget, cancellation, and the duplicate-call fingerprint registry are shared between both agents (one counter, one clock origin, one registry — never a second independent copy); the specialist additionally enforces its own smaller per-delegation bounds (≤15 graph transitions, ≤5 real tool calls). Deployment remains exactly the five containers/services in the table above — this is an internal graph-composition change inside `agent-system-a` only. The public System A API, the final result shape, and the frontend contract are unchanged.
