"""Final Evaluation Checkpoint E.1R §6 -- runs the frozen, independent
50-case holdout (`evaluation/e1r_holdout_cases.json`) against the REAL
Qwen decision provider, AFTER the single supervisor prompt correction.
Same bounded, single-step decision conditions as the original E.1
run (`evaluation/run_e1_live_qwen_eval.py`) -- no tool, MCP, or A2A call
is ever made.

Run with:
    python -m evaluation.run_e1r_holdout_eval

Writes:
    evaluation/e1r_holdout_results.jsonl   (one sanitized record per case)
    evaluation/e1r_holdout_summary.json    (aggregate metrics)

Never saves: complete system prompts, raw model output, chain-of-thought,
credential-bearing URLs, or credential values. Run exactly once per this
checkpoint's own rule -- refuses to silently overwrite an existing
result file.
"""
...
CASES_PATH = Path(__file__).parent / "e1r_holdout_cases.json"
RESULTS_PATH = Path(__file__).parent / "e1r_holdout_results.jsonl"
SUMMARY_PATH = Path(__file__).parent / "e1r_holdout_summary.json"

_FORBIDDEN_SUBSTRINGS = ("chain_of_thought", "chain-of-thought", "authorization", "bearer")
...
def _run_one_case(case: dict, provider: QwenDecisionProvider) -> dict:
    ...
    return {
        "case_id": case["case_id"], "agent_role": role, "language": case["language"], "category": case["category"],
        "expected_actions": expected_actions, "actual_action": actual_action, "correct": correct,
        "first_attempt_schema_valid": first_attempt_valid, "final_schema_valid": final_valid,
        "repair_count": max(0, attempts - 1), "safe_error_code": safe_error_code if not final_valid else "none",
        "latency_seconds": round(latency_seconds, 3), "unnecessary_action": unnecessary_action,
        "model": provider.model, "provider": "qwen", "timestamp": datetime.now(timezone.utc).isoformat(),
        "leak_scan_hits": leak_hits,
    }


def main() -> None:
    if RESULTS_PATH.exists() and "--regenerate" not in sys.argv:
        print(f"REFUSING to overwrite already-recorded holdout results at {RESULTS_PATH}.")
        print("This checkpoint's own rule: execute the frozen holdout exactly once. Pass --regenerate only if you are certain.")
        raise SystemExit(1)

    key_present = qwen_api_key_present()
    print(f"QWEN_API_KEY/DASHSCOPE_API_KEY present: {str(key_present).lower()}")
    if not key_present:
        print("BLOCKED: no Qwen credential present -- holdout evaluation cannot start.")
        raise SystemExit(2)

    manifest = _load_cases()
    print(f"loaded holdout case_set checksum: {manifest['_loaded_checksum']}")
    all_cases = manifest["supervisor_cases"] + manifest["specialist_cases"]
    print(f"total predeclared holdout cases: {len(all_cases)}")
    ...
    summary = {
        ...
        "run_type": "post-fix holdout (independent from the original E.1 baseline)",
