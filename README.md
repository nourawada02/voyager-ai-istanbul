...
**Corpus**: real, stable Wikipedia content in English/Turkish/Arabic (CC BY-SA 4.0) plus a small number of official/primary sources, fetched via `rag/corpus_sources.py`. **`istanbul_rag_B_v2` is the official collection** (see [ADR 0018](docs/adr/0018-rag-official-corpus-promotion.md) for the full official-promotion checkpoint) — both `agent-system-b` (the read path, `QDRANT_COLLECTION_NAME`) and the `rag-ingest` Compose service (the write path) default to it, so a fresh clone's `docker compose run --rm rag-ingest bootstrap` reproducibly populates exactly the collection System B reads. It holds 69 source documents (39 EN / 14 TR / 16 AR) across 17 interest facets, chunked into **287 document-chunk points plus one fingerprint-marker point = 288 Qdrant points total** — verified directly against a real, disposable Qdrant instance. Two earlier collections coexist in the same Qdrant instance, untouched, as legacy rollback/evaluation artifacts: `istanbul_rag_B` (91 points, the original frozen 20-document Phase 3 corpus) and `istanbul_rag_B_r1_shadow` (288 points, the R.1 development shadow collection this checkpoint formally promoted and superseded). Neither is deleted, renamed, or mutated; either can still be queried by setting `QDRANT_COLLECTION_NAME`/`QDRANT_EXPECTED_FINGERPRINT` back explicitly.
...
**RAG-first vs. catalog fallback (measured, `evaluation/datasets/istanbul_expert_r1_summary_report.json`):** across 8 demonstration cases (EN, TR, AR, and accessibility-focused requests), **24 of 27** scheduled points of interest (88.9%) originated from RAG evidence, meeting the project's own 80% RAG-origin target; the remaining 3 came from catalog fallback. A separate Qdrant-unavailable demo confirmed clean, honest degradation: 4/4 candidates fell back to the catalog, with explicit warnings (`candidate_pool_no_rag_candidates_catalog_fallback_used`, `rag_unavailable_structured_catalog_only`) and no fabricated RAG evidence. This 88.9% figure was measured against the R.1 shadow collection's 69-document corpus content — the exact same corpus content `istanbul_rag_B_v2` now serves under its official name; the official-promotion checkpoint changed deployment reproducibility and naming, not the measured retrieval algorithm, so this number was **not** re-run against `istanbul_rag_B_v2` and is not claimed to be.

Ingestion is a real, idempotent one-shot Compose service (`rag-ingest`, `profiles: ["ingest"]`) — it never runs as part of a bare `docker compose up` and must be run explicitly:

```
docker compose run --rm rag-ingest bootstrap   # populates istanbul_rag_B_v2 from the canonical 69-document corpus
docker compose run --rm rag-ingest status      # reports readiness of the configured collection
```

Both the collection name (`QDRANT_COLLECTION_NAME`, default `istanbul_rag_B_v2`) and the corpus-aware ingestion fingerprint (`rag/ingest.py::compute_fingerprint` — corpus identity, chunking config, embedding identity, and schema version, all combined) are configurable and fail closed on any mismatch; a genuinely fresh Qdrant volume reproduces the exact 69-document / 287-chunk / 288-point collection above with one command, no manual or undocumented step required.

**Adding sources is intentionally not yet supported.** `rag-ingest`'s third subcommand, `ingest --source PATH` (incrementally adding documents to an existing collection), is **disabled for this release** — a collection's stored fingerprint is a single, fixed identity for one exact corpus, and this checkpoint has no correct, non-misleading way yet to represent "a canonical collection plus extra documents" as its own distinct identity. It refuses unconditionally, for every collection name, before touching any file or connection. To add sources: define a new, explicitly versioned corpus manifest (extend `rag/corpus_sources.py`) and `bootstrap` a new, separately named versioned collection from it — see [ADR 0018 §4](docs/adr/0018-rag-official-corpus-promotion.md#4-incremental-ingestion-is-disabled-for-this-release).
...
populates `istanbul_rag_B_v2` from the canonical 69-document corpus — the exact collection System B is configured to read by default (see [Qdrant multilingual RAG](#qdrant-multilingual-rag)). Idempotent and safe to re-run; a genuinely fresh `vector-db` volume with this one command run reproduces the full 69-document / 288-point collection. Skipping this step is also safe: System B degrades to catalog-only scheduling with an explicit warning rather than failing or fabricating evidence.
...
**Qdrant collection appears empty / System B shows no citations.** Run `docker compose run --rm rag-ingest bootstrap` — it populates `istanbul_rag_B_v2`, the exact collection System B reads by default (see [Qdrant multilingual RAG](#qdrant-multilingual-rag)); idempotent, safe to re-run. Until it's run against a genuinely fresh `vector-db` volume, System B correctly degrades to catalog-only scheduling (with an explicit warning) rather than failing or fabricating evidence.
