...
**Current state as of 2026-08-14: Phase 0 (compatibility spike) closed. Phase 1 (contracts & fixtures) closed. Phase 2 (data & accommodation ML) Checkpoint A (data and architecture foundation) complete, Checkpoint B (model experimentation and artifact selection) complete, Checkpoint C.0 (accommodation-serving contract and safety design) complete, Checkpoint C.1 (trusted resource loading and pure in-process accommodation-search service) complete. Checkpoint C.2 has not started — no production `search_stays` MCP tool exists yet. Hotel-room and Shared-room listings remain unsupported by the V1 serving contract.** Update this line only when a phase gate is formally passed.
